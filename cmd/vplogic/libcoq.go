...
var libcoq = strings.Join([]string{
	...
	"| ZKSETUP: value -> primitive",
	"| ZKPROVE: value -> value -> value -> primitive",
	"| ZKVERIFY: value -> value -> value -> primitive",
	...
	"| zksetup_constructor(seed out: value): knows seed -> out = prim(ZKSETUP seed) -> knows out",
	"| zkprove_constructor(params stmt wit out: value): knows params -> knows stmt -> knows wit",
	"  -> out = prim(ZKPROVE params stmt wit) -> knows out",
	"| zkverify_constructor(params stmt proof out: value): knows params -> knows stmt -> knows proof",
	"  -> out = prim(ZKVERIFY params stmt proof) -> knows out",
	"| zkverify_rewriterule(params stmt wit: value):",
	"  knows (prim(ZKVERIFY params stmt (prim(ZKPROVE params stmt wit)))) -> knows stmt",
	"",
	"(*secret sharing*)",
	...
	"        | ZKSETUP seed1, ZKSETUP seed2 => value_beq seed1 seed2",
	"        | ZKPROVE params1 stmt1 wit1, ZKPROVE params2 stmt2 wit2 => ((value_beq params1 params2) && (value_beq stmt1 stmt2)) && (value_beq wit1 wit2)",
	"        | ZKVERIFY params1 stmt1 proof1, ZKVERIFY params2 stmt2 proof2 => ((value_beq params1 params2) && (value_beq stmt1 stmt2)) && (value_beq proof1 proof2)",
	...
	"              | _ => prim(UNBLIND k m (apply_rewrites l s))",
	"            end",
	"          | ZKSETUP seed => prim(ZKSETUP (apply_rewrites l seed))",
	"          | ZKPROVE params stmt wit => prim(ZKPROVE (apply_rewrites l params) (apply_rewrites l stmt) (apply_rewrites l wit))",
	"          | ZKVERIFY params stmt proof => match proof with",
	"              | prim p' => match p' with",
	"                  | ZKPROVE params' stmt_proved wit => match value_beq params params' with",
	"                      | true => apply_rewrites l stmt_proved",
	"                      | false => prim(ZKVERIFY (apply_rewrites l params) (apply_rewrites l stmt) (prim(ZKPROVE (apply_rewrites l params') (apply_rewrites l stmt_proved) (apply_rewrites l wit))))",
	"                    end",
	"                  | _ => prim(ZKVERIFY (apply_rewrites l params) (apply_rewrites l stmt) (apply_rewrites l proof))",
	"                end",
	"              | _ => prim(ZKVERIFY (apply_rewrites l params) (apply_rewrites l stmt) (apply_rewrites l proof))",
	"            end",
