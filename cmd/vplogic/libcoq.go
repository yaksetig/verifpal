...
var libcoq = strings.Join([]string{
	...
	"| ZK_SETUP: value -> primitive",
	"| ZK_PROVE: value -> value -> value -> primitive",
	"| ZK_VERIFY: value -> value -> value -> primitive",
	...
	"| xor_constructor (a b out: value) : knows a -> knows b -> out = prim(XOR a b) -> knows out",
	...
	"| zksetup_constructor(seed out: value): knows seed -> out = prim(ZK_SETUP seed) -> knows out",
	"| zkprove_constructor(params stmt wit out: value): knows params -> knows stmt -> knows wit",
	"  -> out = prim(ZK_PROVE params stmt wit) -> knows out",
	"| zkverify_constructor(params stmt proof out: value): knows params -> knows stmt -> knows proof",
	"  -> out = prim(ZK_VERIFY params stmt proof) -> knows out",
	"| zkverify_rewriterule(params stmt wit: value):",
	"  knows (prim(ZK_VERIFY params stmt (prim(ZK_PROVE params stmt wit)))) -> knows stmt",
	"",
	"(*secret sharing*)",
	...
	"        | XOR a1 b1, XOR a2 b2 => (value_beq a1 a2) && (value_beq b1 b2)",
	...
	"        | ZK_SETUP seed1, ZK_SETUP seed2 => value_beq seed1 seed2",
	"        | ZK_PROVE params1 stmt1 wit1, ZK_PROVE params2 stmt2 wit2 => ((value_beq params1 params2) && (value_beq stmt1 stmt2)) && (value_beq wit1 wit2)",
	"        | ZK_VERIFY params1 stmt1 proof1, ZK_VERIFY params2 stmt2 proof2 => ((value_beq params1 params2) && (value_beq stmt1 stmt2)) && (value_beq proof1 proof2)",
	...
	"        | XOR a b => (has_nested_value a v) || (has_nested_value b v)",
	...
	"      | XOR a b => has_password a ++ has_password b",
	...
	"            | false, false => new",
	"          end",
	"        | XOR a1 b1, XOR a2 b2 => match value_beq a1 a2, value_beq b1 b2 with",
	"            | true, true => old",
	"            | false, true => prim(XOR a2 b1)",
	"            | true, false => prim(XOR a1 b2)",
	"            | false, false => new",
	"          end",
	...
	"          | XOR a b => prim(XOR (apply_rewrites l a) (apply_rewrites l b))",
	...
	"              | _ => prim(UNBLIND k m (apply_rewrites l s))",
	"            end",
	"          | ZK_SETUP seed => prim(ZK_SETUP (apply_rewrites l seed))",
	"          | ZK_PROVE params stmt wit => prim(ZK_PROVE (apply_rewrites l params) (apply_rewrites l stmt) (apply_rewrites l wit))",
	"          | ZK_VERIFY params stmt proof => match proof with",
	"              | prim p' => match p' with",
	"                  | ZK_PROVE params' stmt_proved wit => match value_beq params params' with",
	"                      | true => apply_rewrites l stmt_proved",
	"                      | false => prim(ZK_VERIFY (apply_rewrites l params) (apply_rewrites l stmt) (prim(ZK_PROVE (apply_rewrites l params') (apply_rewrites l stmt_proved) (apply_rewrites l wit))))",
	"                    end",
	"                  | _ => prim(ZK_VERIFY (apply_rewrites l params) (apply_rewrites l stmt) (apply_rewrites l proof))",
	"                end",
	"              | _ => prim(ZK_VERIFY (apply_rewrites l params) (apply_rewrites l stmt) (apply_rewrites l proof))",
	"            end",
